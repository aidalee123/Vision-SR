            ...
            for beam in [30]:
                ...
                for _ in range(10):  # Iterations
                    ...
                    try:
                        ...
                        # 1. Evaluate on Training Data (for Selection)
                        # We use the X_padded (the subsample used for training) to calc Train R2
                        X_train_dict = {var: X_padded[:, idx].cpu() for idx, var in enumerate(variables)}
                        y_pre_train = func(**X_train_dict)
                        # Handle complex numbers / NaNs for train
                        y_pre_train = np.nan_to_num(y_pre_train.real if np.iscomplexobj(y_pre_train) else y_pre_train, nan=0.0)
                        
                        r2_train = r2_score(y.cpu().numpy(), y_pre_train)

                        # 2. Evaluate on Test Data (for Reporting only)
                        X_test_dict = {var: X_test_padded[:, idx].cpu() for idx, var in enumerate(variables)}
                        y_pre_test = func(**X_test_dict)
                        # Handle complex numbers / NaNs for test
                        ...
                        r2_train, r2_test, sr, comp, pre_expr = 0, 0, 0, np.inf, "-"
